...
import com.kubbo.rpc.{EchoTestImpl, EchoTest}
...
object ProviderContainerTest {
  ...
  def startProvider = {
    ...
    val test = pContainer.start(classOf[EchoTest],new EchoTestImpl(),"test","1.0.0")
    println(test)
  }

  def startConsumer = {
    val ref = Reference.get()
    val test = ref.getRef(classOf[EchoTest],"test","1.0.0")
    Thread.sleep(3000)
    for(i <- 1 to 10){
